...
if __name__ == '__main__':
    ...
    # transformed_corners = cv2.perspectiveTransform(four_corners, inv_mats)[0][0].astype('int64')

    # transformed = cv2.warpPerspective(img.copy(), inv_mats, (200, 500))

    # transformed_corners = []
    # for p in list(four_corners):
    #     # p = np.array(p,dtype='float64')
    #     # p = np.array([p[0].item(), p[1].item()]).reshape((1, 1, 2))
    #     # print(p)
    #     # transformed = cv2.perspectiveTransform(p, inv_mats, (200, 500))[0][0]
    
    #     cv2.circle(img, (p[0], p[1]), 35, (0,255,255), -1)
        # transformed_corners.append(transformed)

    # max_x, max_y = np.array(transformed_corners)[:,0].max(), np.array(transformed_corners)[:,1].max()

    # print(max_x, max_y)
    # blank_img = np.zeros((int(max_y), int(max_x), 3),dtype =np.uint8)
    # for i in transformed_corners:
    #     cv2.circle(blank_img, (int(i[0]), int(i[1])), 5, (0,255,255), -1)
